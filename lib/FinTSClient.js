...
var FinTSClient = function(in_blz, in_kunden_id, in_pin, in_bankenlist, logger) {
  ...
  // Logger
  me.log = {
    'main': (logger ? logger : bunyan.createLogger({
  ...
  me.MsgInitDialog = function(cb) {
    ...
    if (me.kunden_id != 9999999999) msg.sign({
      'pin': me.pin,
      'tan': NULL,
      'sys_id': me.sys_id,
    ...
    me.SendMsgToDestination(msg, function(error, recvMsg) {
      if (error) {
        ...
        if (HIRMG != null && (HIRMG.getEl(1).getEl(1) == "0010" || HIRMG.getEl(1).getEl(1) == "3060" || HIRMG.getEl(1).getEl(1) == "0020" )) {

          /* debug */
          /*
          console.log('HKVVB');
          _.each(recvMsg.selectSegByNameAndBelongTo("HIRMS",HKVVB.nr), function(segment) {
            _.each(_.get(segment, 'store.data'), function(dataElement) {
               console.log(dataElement.getEl(1));
            });
          ...
          */
          //kontolist
          /*
          _.each(recvMsg.selectSegByName("HIUPD"), function(segment) {
            console.log(JSON.stringify(segment));
          });
          */


          if (Helper.checkMsgsWithBelongToForId(recvMsg, HKVVB.nr, "0020") || true) {
            try {
